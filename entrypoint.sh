...
if [ -z "$INPUT_EXCLUDE_DIR" ]; then
    ...
    eval clang-tidy-12 -p="$(pwd)" "$INPUT_CLANG_TIDY_ARGS" -- >(tee "clang_tidy.txt")
else
    ...
    clang-tidy-12 -p="$(pwd)" "$INPUT_CLANG_TIDY_ARGS" "^((?!$GITHUB_WORKSPACE/$INPUT_EXCLUDE_DIR).)*$" -- > clang_tidy.txt
fi
